...
const CartList = ({ products, changeQuantity }: cartItemsProps) => {
    return (
        ...
        <>
            {
                products.map((product) => (
                    <Cartitem key={product.id} id={product.id} title={product.title} price={product.price} img={product.img} quantity={product.quantity} max={product.max} changeQuantity={changeQuantity} />
                ))
            }
        </>
    )
